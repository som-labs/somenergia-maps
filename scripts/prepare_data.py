...
idir = '../data/distribution-summaries/'  # Name of input directory where all csv files are (and nothing else).
odir = '../data/'  # Name of output directory.
...
for file_i in xrange(len(listfiles)):
    ...
    data = pd.read_csv(idir + listfiles[file_i], sep=None, header=0, usecols=['codi_ine', 'quants'], engine='python',
                       dtype={'codi_ine': str, 'quants': int})
